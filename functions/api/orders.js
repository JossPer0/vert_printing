...
function buildCustomerOrderText(order, items) {
  const firstName = clean(order.customer_name, 160).split(/\s+/)[0] || "there";
  return [
    `Thanks, ${firstName}!`,
    "",
    "We've received your Vert Printing order request.",
    "",
    ...
    ...items.map((item) => {
      const options = (item.options_snapshot || []).map((option) => `  ${option.label}`).join("\n");
      return `${item.product_name_snapshot}\nQty ${item.quantity} - ${money(item.line_total)}${options ? `\n${options}` : ""}`;
    }),
    "",
    `ORDER TOTAL: ${money(order.grand_total)}`,
    "",
    "WHAT HAPPENS NEXT?",
    "We'll review your order and confirm any outstanding artwork, payment and production details with you.",
    "Nothing will go into production until everything has been confirmed.",
    "",
    "Need help?",
    "Reply to this email or contact Vert Printing.",
    "Email: info@vertprinting.co.za",
    "Phone / WhatsApp: +27 66 245 6511",
    "Website: https://www.vertprinting.co.za/",
    "",
    "Vert Printing, Kloof, Durban",
  ].join("\n");
}
      ...
      <head>
        ...
        <style>
          @media only screen and (max-width: 520px) {
            ...
            .email-total-price { font-size: 19px !important; }
          }
        </style>
      </head>
                    ...
                    <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;">
                      <tr>
                        <td style="vertical-align:middle;color:#667074;font-family:Arial,Helvetica,sans-serif;font-size:13px;line-height:1.35;">
                          <img src="https://www.vertprinting.co.za/assets/vert_logo.png" width="112" alt="Vert Printing" style="display:block;width:112px;max-width:112px;height:auto;border:0;margin:0 0 8px;">
                          <strong style="display:block;color:#1f2426;font-size:14px;letter-spacing:.06em;">VERT PRINTING</strong>
                          Kloof, Durban
                        </td>
                      </tr>
                    </table>
                    ...
                    <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;table-layout:fixed;">
                      <tr>
                        <td class="email-pad" style="padding:30px 28px 18px;border-bottom:1px solid #ece8df;">
                          <div style="margin:0 0 10px;color:#ec168c;font-family:Arial,Helvetica,sans-serif;font-size:12px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;">Order request received</div>
                          <h1 class="email-word" style="margin:0 0 12px;color:#1f2426;font-family:Arial,Helvetica,sans-serif;font-size:28px;line-height:1.15;word-break:break-word;">Thanks, ${firstName}!</h1>
                          <p class="email-word" style="margin:0;color:#4f5b5f;font-family:Arial,Helvetica,sans-serif;font-size:16px;line-height:1.6;word-break:break-word;">We've received your Vert Printing order request.</p>
                        </td>
                      </tr>
                      <tr>
                        <td class="email-pad" style="padding:20px 28px;background:#fbfaf7;border-bottom:1px solid #ece8df;">
                          <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;table-layout:fixed;">
                            <tr><td style="padding:0;color:#1f2426;font-family:Arial,Helvetica,sans-serif;font-size:16px;line-height:1.4;word-break:break-word;"><strong>Order #${orderNumber}</strong><div style="margin-top:10px;"><span style="display:inline-block;border:1px solid #b8d6d4;border-radius:999px;padding:6px 10px;color:#005c5a;background:#eef8f7;font-family:Arial,Helvetica,sans-serif;font-size:12px;font-weight:700;line-height:1;">Awaiting confirmation</span></div></td></tr>
                          </table>
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td class="email-pad" style="padding:18px 28px 24px;">
                          <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;background:#1f2426;border-radius:8px;table-layout:fixed;">
                            <tr>
                              ...
                              <td class="email-total-price" align="right" style="padding:18px 20px;color:#ffffff;font-family:Arial,Helvetica,sans-serif;font-size:22px;font-weight:700;white-space:nowrap;">${money(order.grand_total)}</td>
                            </tr>
                          </table>
                        </td>
                      </tr>

...
export default function AdminApp() {
  ...
  async function loadData(client = supabase) {
    ...
    let [categoryResult, productResult, imageResult, productCategoryResult, specificationResult, modelResult] = await Promise.all([
      ...
      client.from('product_model_files').select('product_id,original_filename,format,file_size_bytes,unit,width,depth,height,volume,surface_area,triangle_count,object_count,watertight').order('updated_at', { ascending: false }),
    ]);
    ...
    if (!modelResult.error) {
      ...
      for (const model of modelResult.data || []) grouped[model.product_id] = { ...model, filename: model.original_filename } as ModelAnalysis;

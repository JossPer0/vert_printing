...
type OptionValue = { id?: string; option_group_id?: string; label: string; value: string; price_adjustment: number; sort_order: number; is_active: boolean };
...
const blankGroup = (): OptionGroup => ({ name: '', display_type: 'select', is_required: false, sort_order: 0, values: [{ label: '', value: '', price_adjustment: 0, sort_order: 0, is_active: true }] });
    ...
    try {
      ...
      for (const [groupIndex, group] of groups.entries()) {
        ...
        for (const [valueIndex, value] of validValues.entries()) {
          const { error: valueError } = await supabase.from('option_values').upsert({ id: value.id, option_group_id: saved.id, label: value.label.trim(), value: value.value.trim(), price_adjustment: Number(value.price_adjustment) || 0, sort_order: valueIndex, is_active: value.is_active }, { onConflict: 'id' });

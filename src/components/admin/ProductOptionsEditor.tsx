...
export default function ProductOptionsEditor({ productId, supabase }: { productId?: string; supabase: SupabaseClient | null }) {
  ...
  return <div className="admin-card admin-card-wide"><div className="admin-section-heading"><div><h2>Product Choices</h2><p className="admin-muted">Add the choices customers must make before ordering, such as outer colour, inner colour or size.</p></div><button className="admin-button primary" type="button" onClick={() => setGroups([...groups, blankGroup()])} disabled={!productId}>Add another choice</button></div>{!productId && <p className="admin-muted">Save the product first, then add selectable options.</p>}{productId && groups.map((group, groupIndex) => <div className="admin-option-group" key={group.id || groupIndex}><div className="admin-option-group-heading"><div><span className="admin-option-number">Choice {groupIndex + 1}</span><h3>{group.name || 'New choice'}</h3></div><span className="admin-muted">Customer selection</span></div><div className="admin-field-grid"><label className="admin-field"><span>Choice name</span><input value={group.name} placeholder="Colour or Size" onChange={(event) => updateGroup(groupIndex, { name: event.target.value })} /></label><label className="admin-field"><span>How customers choose</span><select value={group.display_type} onChange={(event) => updateGroup(groupIndex, { display_type: event.target.value as OptionGroup['display_type'] })}><option value="select">Dropdown</option><option value="radio">Choice buttons</option><option value="swatch">Colour swatches</option></select></label></div><label className="admin-toggle"><input type="checkbox" checked={group.is_required} onChange={(event) => updateGroup(groupIndex, { is_required: event.target.checked })} /><span>Customer must choose a value</span></label><div className="admin-option-values"><div className="admin-option-value-head"><span>Choice label</span><span>Price adjustment</span><span>Colour</span><span></span></div>{group.values.map((value, valueIndex) => <div className={`admin-option-value ${group.name.trim().toLowerCase() === 'colour' && group.display_type === 'swatch' ? 'has-picker' : ''}`} key={value.id || valueIndex}><input aria-label="Option label" placeholder="Customer-facing label" value={value.label} onChange={(event) => updateValue(groupIndex, valueIndex, { label: event.target.value })} /><input aria-label="Price adjustment" type="number" step="0.01" value={value.price_adjustment} onChange={(event) => updateValue(groupIndex, valueIndex, { price_adjustment: Number(event.target.value) })} />{group.name.trim().toLowerCase() === 'colour' && group.display_type === 'swatch' ? <input className="admin-colour-picker" aria-label="Colour" type="color" value={value.metadata?.color || '#ec168c'} onChange={(event) => updateValue(groupIndex, valueIndex, { metadata: { ...value.metadata, color: event.target.value } })} /> : <span className="admin-colour-placeholder" aria-hidden="true" />}<button className="admin-button danger" type="button" onClick={() => updateGroup(groupIndex, { values: group.values.filter((_, current) => current !== valueIndex) })}>Remove</button></div>)}</div><div className="admin-option-actions"><button className="admin-button secondary" type="button" onClick={() => updateGroup(groupIndex, { values: [...group.values, { label: '', value: '', price_adjustment: 0, sort_order: group.values.length, is_active: true, metadata: { color: '#ec168c' } }] })}>Add another value</button><button className="admin-button danger" type="button" onClick={() => removeGroup(groupIndex)}>Remove group</button></div></div>)}{productId && groups.length > 0 && <div className="admin-option-save"><button className="admin-button primary" type="button" onClick={save} disabled={busy}>{busy ? 'Saving...' : 'Save options'}</button>{message && <span className="admin-muted" role="status">{message}</span>}</div>}</div>;
}